...
import { usePetCafeViewModel } from '../hooks/usePetCafeViewModel';
import { PetCafeCard } from '../components/PetCafeCard';
import LoadingSkeleton from '@/core/components/LoadingSkeleton';

const PetCafePage: React.FC = () => {
  const { t } = useTranslation();
  const { data, isLoading, error } = usePetCafeViewModel();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data?.map((item) => (
          <PetCafeCard key={item.id} item={item} />
        ))}
        {(!data || data.length === 0) && (
