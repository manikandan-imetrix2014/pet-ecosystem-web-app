...
import { useMatingViewModel } from '../hooks/useMatingViewModel';
import { MatingCard } from '../components/MatingCard';
import LoadingSkeleton from '@/core/components/LoadingSkeleton';

const MatingPage: React.FC = () => {
  const { t } = useTranslation();
  const { data, isLoading, error } = useMatingViewModel();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data?.map((item) => (
          <MatingCard key={item.id} item={item} />
        ))}
        {(!data || data.length === 0) && (
