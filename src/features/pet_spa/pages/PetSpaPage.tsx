...
import { usePetSpaViewModel } from '../hooks/usePetSpaViewModel';
import { PetSpaCard } from '../components/PetSpaCard';
import LoadingSkeleton from '@/core/components/LoadingSkeleton';

const PetSpaPage: React.FC = () => {
  const { t } = useTranslation();
  const { data, isLoading, error } = usePetSpaViewModel();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data?.map((item) => (
          <PetSpaCard key={item.id} item={item} />
        ))}
        {(!data || data.length === 0) && (
